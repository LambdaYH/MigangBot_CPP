#ifndef MIGANGBOT_DATABASE_MYSQL_CONN_POOL_H_
#define MIGANGBOT_DATABASE_MYSQL_CONN_POOL_H_
...
inline void MySQLConnPool::Init(const std::string &host,
  ...
  for (std::size_t i = 0; i < conn_num; ++i) {
    ...
    if (mysql_options(&sql_conn_pool_[i], MYSQL_OPT_RECONNECT,
                      (bool *)&reconnect_opt)) {
      LOG_ERROR("数据库连接初始化失败");
      exit(3);
    }

    if (!mysql_real_connect(&sql_conn_pool_[i], host.c_str(), user.c_str(),
...
inline MySQLConnPool::~MySQLConnPool() {
  for (auto &conn : sql_conn_pool_) mysql_close(&conn);
  mysql_library_end();
}
