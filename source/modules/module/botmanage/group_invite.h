#ifndef MIGANGBOT_MODULES_MODULE_GROUP_INVITE_H_
#define MIGANGBOT_MODULES_MODULE_GROUP_INVITE_H_
...
public:
        ...
        auto_leave_reason_(config_["自动退群理由"].as<std::string>())
    {
        ...
        botmanage::reject_group_force_flag = config_["是否自动退出被迫自动入的群"].as<bool>();
    }
    virtual void Register() override
    {
        RegisterRequest("group", "invite", ACT(GroupInvite::Handle_group_invite));
        RegisterNotice("group_increase", "", ACT(GroupInvite::Handle_group_increase));
    }
...
inline void GroupInvite::Handle_group_invite(const Event &event, onebot11::ApiBot &bot)
{
    ...
    if(botmanage::add_group_flag)
    {
        ...
        {
            bot.reject(event, fmt::format("黑名单:{}", botmanage::IsBlackUser(user_id) ? user_id : group_id));
            auto user_info = bot.get_stranger_info(user_id).Ret();
            for(auto white : config::SUPERUSERS)
                    bot.send_private_msg(white, fmt::format(
                        "{}|{}已拒绝加入(群){}，邀请人{}({})(黑名单)",
...
inline void GroupInvite::Handle_group_increase(const Event &event, onebot11::ApiBot &bot)
{
    auto user_id = event["user_id"].get<QId>();
    auto group_id = event["group_id"].get<GId>();
