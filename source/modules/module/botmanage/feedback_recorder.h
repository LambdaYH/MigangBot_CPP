...
class FeedbackRecorder {
 public:
  ...
  std::size_t GetLastID() {
    ...
    if (one_row.empty() || one_row[0].empty()) return 0;
  ...
  bool RecordFeedBack(const std::string &time, QId uid, GId gid,
    ...
    auto query = fmt::format(
        "INSERT INTO Feedbacks(time, UID, GID, content)\n"
        "VALUES(\n"
        "\"{}\",\n"
        "{},\n"
        "{},\n"
        "\"{}\")",
        time, uid, gid, content);
    int ec;
    sql_wrapper.Execute(query, &ec);
    if (ec) {
  ...
  std::vector<std::string> GetFeedback(const std::string &feedback_id) {
    return sql::MySQLWrapper()
        .Execute(fmt::format(
            "SELECT time, content, UID, GID FROM Feedbacks WHERE feedbackID={}",
            feedback_id))
        .FetchOne();
  }
};
