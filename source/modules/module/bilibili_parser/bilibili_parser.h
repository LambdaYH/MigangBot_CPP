...
private:
    ...
    std::string GetBilibiliBangumiDetail(const std::string &url, const GId group_id);
...
inline bool IsInCache(const std::string &url, const GId group_id)
{
    ...
    if(!redis_wrapper.Execute(fmt::format("GET {}", fmt::format("bnotrepeat:{}:{}", group_id, url))))
        return false;
...
{
    redis::RedisWrapper redis_wrapper;
    if(!redis_wrapper.Execute(fmt::format("SET {} 0 EX {}", fmt::format("bnotrepeat:{}:{}", group_id, url), kExpiredTime)))
        return false;
    return true;
}
...
{
    redis::RedisWrapper redis_wrapper;
    if(!redis_wrapper.Execute(fmt::format("SET {} {} EX {}", fmt::format("bcache:{}", url), value, kExpiredTimeCache)))
        return false;
    return true;
}
...
{
    redis::RedisWrapper redis_wrapper;
    if(!redis_wrapper.Execute(fmt::format("GET {}", fmt::format("bcache:{}", url))))
        return "";
    if(redis_wrapper.GetReply().type == REDIS_REPLY_NIL)
        return "";
...
inline std::string BilibiliParser::GetBilibiliVideoDetail(std::string &url, const GId group_id)
{
...
inline std::string BilibiliParser::GetBilibiliBangumiDetail(const std::string &url, const GId group_id)
{
    auto cache = GetFromCache(url);
    if(!cache.empty())
    {
...
inline std::string BilibiliParser::GetLiveSummary(const std::string &url, const GId group_id)
{
    ...
    static std::regex link_search_pattern(R"((https|http)://live.bilibili.com/\d+)");
    ...
    auto r = GetJson(fmt::format("http://api.live.bilibili.com/room/v1/Room/room_init?id={}", link_match[0].str()));
    if(!r.contains("code") || r["code"].get<int>() != 0)
    {
        AddInCacheNotRepeat(link, group_id);
        return "↑ 直播间不存在~";
    }
    r = GetJson(fmt::format("http://api.bilibili.com/x/space/acc/info?mid={}", r["data"]["uid"].get<int>()));
    if(!r.contains("code") || r["code"].get<int>() != 0)
    {
    ...
    auto msg = fmt::format(
        ...
        message_segment::image(r["data"]["live_room"]["cover"].get<std::string>()),
        link
    );
    AddInCache(url, msg);
    AddInCacheNotRepeat(link, group_id);
...
inline std::string BilibiliParser::ExtractDetails(const std::string &url, const GId group_id)
{
...
inline void BilibiliParser::Parser(const Event &event, onebot11::ApiBot &bot)
{
    ...
    if(url_list_set.empty())
    {
        ...
        auto bvid_begin = std::sregex_iterator(msg.begin(), msg.end(), bvid_pattern_);
        for(; aid_begin != search_end; ++aid_begin)
            url_list_set.insert(fmt::format("https://www.bilibili.com/video/{}", (*aid_begin).str()));
        for(; bvid_begin != search_end; ++bvid_begin)
            url_list_set.insert(fmt::format("https://www.bilibili.com/video/{}", (*bvid_begin).str()));
    }
    if(url_list_set.empty())
        return;
