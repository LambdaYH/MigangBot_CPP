...
class EorzeaZhanbu : public Module {
 public:
  EorzeaZhanbu()
      : Module("zhanbu.yml", eorzea_zhanbu::kConfigExample),
        exception_msg_({"占卜水晶球滑落了~正在重新捡起(擦擦)",
                        "水晶球化为碎片，正在尝试重组~", "星天开门！"}) {
    ...
    for (auto &event : events_) {
      if (config_["宜"][event]) {
        for (std::size_t i = 0; i < config_["宜"][event].size(); ++i)
          luck_yi_reply_[event].push_back(
              config_["宜"][event][i].as<std::string>());
      }
      if (config_["忌"][event]) {
        for (std::size_t i = 0; i < config_["忌"][event].size(); ++i)
          luck_ji_reply_[event].push_back(
              config_["忌"][event][i].as<std::string>());
      }
    }
  ...
  virtual void Register() {
    RegisterCommand(PREFIX, {"/zhanbu", "/占卜", "、占卜"},
                    ACT(EorzeaZhanbu::Zhanbu), permission::NORMAL);
  }
...
inline std::string EorzeaZhanbu::GetEorzeaZhanbu(const QId uid) {
  ...
  if (record.empty() || std::stoll(record.back()) <= cur_time) {
    ...
    recorder_.RecordZhanbu(uid, luck, yi, ji, dye, append_msg, basemap,
                           datetime::LastSecondOfToday());
...
EorzeaZhanbu::GetZhanbuResult(const QId uid) {
  ...
  auto luck_occupation =
      *select_randomly(occupations_.begin(), occupations_.end());
  ...
  if (luck == 100)
    ...
    if (GetRandomNum(0, 1) == 0)
      append_msg = *select_randomly(luck_yi_reply_.at(luck_yi).begin(),
                                    luck_yi_reply_.at(luck_yi).end());
    else
      append_msg = *select_randomly(luck_ji_reply_.at(luck_ji).begin(),
                                    luck_ji_reply_.at(luck_ji).end());
...
inline std::string EorzeaZhanbu::GetBasemap(const std::string &occupation) {
  return *select_randomly(occupation_basemaps_.at(occupation).begin(),
                          occupation_basemaps_.at(occupation).end());
}
