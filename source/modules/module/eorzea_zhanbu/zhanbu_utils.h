...
inline int GetLuckNum(const QId uid) {
  ...
  MD5((const unsigned char *)str_for_md5.c_str(), str_for_md5.size(),
      (unsigned char *)md5.data());
...
inline std::string GetEventZhanbu(const QId uid, const std::string &thing) {
  ...
  auto today_int = std::stoi(fmt::format("{:%Y%m%d}", *std::localtime(&t)));
  auto str_for_md5 =
      fmt::format("{}{}{}", thing.size() * today_int, today_int * uid, thing);
  std::string md5('\0', MD5_DIGEST_LENGTH);
  MD5((const unsigned char *)str_for_md5.c_str(), str_for_md5.size(),
      (unsigned char *)md5.data());
  std::mt19937 e(std::hash<std::string>()(md5));
...
inline std::vector<std::u16string> Decrement(const std::string &text_u8) {
  ...
  auto cardinality = 9;
  if (length > 4 * cardinality) return {};
  auto number_of_slices = (length % cardinality == 0)
                              ? length / cardinality
                              : length / cardinality + 1;

  if (number_of_slices == 2) {
    if (length & 1) {
      std::u16string fill_space(cardinality - (length + 1) / 2, ' ');
      return {
          text.substr(0, (length + 1) / 2) + fill_space,
...
inline std::string Draw(const std::string &luck, const std::string &yi,
                        const std::string &ji, const std::string &dye,
                        const std::string &append_msg,
                        const std::string &basemap) {
  static const auto kTitleFont =
      config::kAssetsDir / "fonts" / "zhanbu" / "Mamelon.otf";
  static const auto kTextFont =
      config::kAssetsDir / "fonts" / "zhanbu" / "sakura.ttf";
  ...
  freetype2->putText(img, luck,
                     cv::Point(image_font_center.first - font_size.width / 2,
                               image_font_center.second - font_size.height / 2),
                     font_height, color, -1, CV_AA, false);
  // draw dye
  font_height = 18;
  ...
  for (std::size_t k = 0; k < 3; ++k) {
    ...
    if (text_slice.empty()) return "";
    auto slice_num = text_slice.size();
    for (std::size_t i = 0; i < slice_num; ++i) {
      auto h = text_slice[i].size() * (font_height + 4);
      auto ori_x = image_font_center.first + (slice_num - 2) * font_height / 2 +
                   (slice_num - 1) * 4 - i * (font_height + 4);
      auto ori_y = image_font_center.second - h / 2;
      for (std::size_t j = 0; j < text_slice[i].size(); ++j) {
        freetype2->putText(img, utf8::utf16to8(text_slice[i].substr(j, 1)),
                           cv::Point(ori_x, ori_y), font_height, color, -1,
                           CV_AA, false);
        ori_y += font_height + 4;
      }
    }
