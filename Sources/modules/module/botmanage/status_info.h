#ifndef MIGANGBOTCPP_MODULES_MODULE_BOTMANAGE_STATUS_INFO_H_
...
namespace status_info
{
    ...
    inline std::string GetMemoryStatus()
    {
        ...
        long long virtual_mem_used = mem_info.totalram - mem_info.freeram - mem_info.bufferram;
        virtual_mem_used += mem_info.totalswap - mem_info.freeswap;
        virtual_mem_used *= mem_info.mem_unit;

        return fmt::format("[内存] 使用率: {:0.1f}%", 100 * ((double)virtual_mem_used / total_virtual_mem));
    }  
    ...
    inline std::string GetNetworkStatus()
    {
        ...
        return fmt::format("[网络] Baidu: {} | Google: {}", baidu_status, google_status);
    }
    ...
    inline std::string GetDiskStatus()
    {
        ...
        return fmt::format("[硬盘] 使用率：{:0.2f}% 剩余空间：{:0.2f}G", 100 * (space_cap - space_avl) / space_cap, space_avl / 1024);
    }
}
...
public:
        ...
        Module()
    {
        ...
        fscanf(file, "cpu %llu %llu %llu %llu", &last_total_user_, &last_total_user_low,
            &last_total_sys_, &last_total_idle_);
        fclose(file);
    }
...
inline std::string StatusInfo::GetCPUStatus()
{
    ...
    if (total_user < last_total_user_ || total_user_low < last_total_user_low ||
        ...
        total = (total_user - last_total_user_) + (total_user_low - last_total_user_low) +
