...
nlohmann::json set_echo(const nlohmann::json& t) {
  nlohmann::json ret_message{{"data", {{"message_id", 1023027117}}},
                             {"echo", t["echo"]},
                             {"retcode", 0},
                             {"status", "ok"}};
  return ret_message;
...
int main(int argc, char** argv) {
  ...
  nlohmann::json message{{"anonymous", nullptr},
                         {"font", 0},
                         {"group_id", 855134081},
  ...
  for (std::size_t i = 0; i < thread_num; ++i) {
    threads.push_back(
        std::thread{[loop_count, &msg_str, &url, i, &count_per_thread]() {
          WebSocketClient ws;
  ...
  std::cout << "Success: "
            << std::accumulate(count_per_thread.begin(), count_per_thread.end(),
                               0)
            << std::endl;
